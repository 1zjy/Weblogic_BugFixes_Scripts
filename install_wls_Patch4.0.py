...
logname="/opt/wlspatch" + datetime.datetime.now().strftime("%Y%m%d")+"_log.txt"
...
def check_wlsPatch(wls_home,bsu_home):
 try:
    ...
    check_cmd=('su - weblogic -c "cd %s;./bsu.sh -prod_dir=%s -status=applied -verbose -view|grep \'Patch ID\'|cut -d \':\' -f 2|sed s/[[:space:]]//g"' % (bsu_home,wls_home))
...
def remove_wlsPatch(wls_home,bsu_home):
 try:
    mark_file(logname,'a',">>>>>>>>移除版本前检查!!<<<<<<<<<<<<")
    print(">>>>>>>>移除版本前检查!!<<<<<<<<<<<<")
    currentPatch=check_wlsPatch(wls_home,bsu_home)
    if currentPatch:
        print(">>>>>>>>开始移除weblogic Patch版本：%s" % currentPatch)
        mark_file(logname,'a',">>>>>>>>开始移除weblogic Patch版本：%s" % currentPatch)
        check_cmd=('su - weblogic -c  "cd %s;./bsu.sh -remove -patchlist=%s -prod_dir=%s"'  % (bsu_home,currentPatch,wls_home))
        mark_file(logname,'a',">>>>>>>>当前执行命令:%s" % check_cmd)
        remove_str=commands.getoutput(check_cmd)
        print ">>>>>>>>移除进程:\n%s" % remove_str
        mark_file(logname,'a',">>>>>>>>移除进程:\n%s" % remove_str)
        print(">>>>>>>>移除weblogic Patch:%s 成功完成！！" % currentPatch)
        mark_file(logname,'a',">>>>>>>>移除weblogic Patch:%s 成功完成！！" % currentPatch)
        return 0
    else:
...
def Install_wlsPatch(wls_home,bsu_home,New_wlsPatch,patch_file):
 try:
    ...
    if NewcurrentPatch:
       if NewcurrentPatch==New_wlsPatch:
          ...
          print(">>>>>>>>当前weblogic Patch已经是:%s,跳过当前安装进程，执行更新操作！！" % NewcurrentPatch)
          mark_file(logname,'a',">>>>>>>>当前weblogic Patch已经是:%s,跳过当前安装进程，执行更新操作！！" % NewcurrentPatch)
          Update_wlsPatch(wls_home,bsu_home,New_wlsPatch,patch_file)
    else:
        try:
...
#函数四：更新weblogic Patch版本
def Update_wlsPatch(wls_home,bsu_home,New_wlsPatch,patch_file):
 try:
    ...
    if current_wlsVersion:
        if current_wlsVersion == New_wlsPatch:
           ...
           remove_wlsPatch(wls_home,bsu_home)
           Install_wlsPatch(wls_home,bsu_home,New_wlsPatch,patch_file)
...
def wls_help(scriptsName):
    print ('''
              ...
              python %s [oper][mode] [wls_home] [bsu_home]
                                       ...
                                       --remove:  卸载当前weblogic Patch版本   
                                       --install  [..] [新补丁名] [新wls包名] :安装weblogic Patch版本        (前提是卸载之前的版本，否则跳过当前安装，进入update功能)
                                       --update   [..] [新补丁名] [新wls包名] :更新weblogic Patch版本 
                        其中oper{hand:手动指定目录,auto:自动通过启动java进程获取目录,其中前提是启动好服务的}
                        例如：(自动安装：weblogic最新漏洞补丁：python %s auto --update U5I2 p29204678_1036_Generic.zip)
                              (手动安装: weblogic最新漏洞补丁：python %s hand --update '/u01/middleware/wlserver_10.3' '/u01/middleware/utils/bsu' U5I2 p29204678_1036_Generic.zip )  
             -----------------------------------------------------------------------------------------------------------------------------------------------------------------------          
          ''' % (scriptsName,scriptsName,scriptsName))
def main():
    mark_file(logname,'w',"**********************************************************Weblogic Patch版本修复报告*****************************************************************")
    mark_file(logname,'a',"修复时间：%s                                                                                 " % datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
    modes=['--help','--check','--update','--remove','--install']
    wls_home=''
    bsu_home=''
    New_wlsPatch=''
    patch_file=''
