 ...
 try:
    ...
    if currentPatch:
       ...
       return 0
        ...
        print(">>>>>>>>当前没有可卸载的weblogic Patch版本")
        mark_file(logname,'a',">>>>>>>>当前没有可卸载的weblogic Patch版本")
 except subprocess.CalledProcessError as err:
      print err.output
      return err.returncode

#函数3：安装weblogic Patch版本
def Install_wlsPatch(wls_home,bsu_home,New_wlsPatch,patch_file,version,isremove):
 try:
    if isremove=='1':
...
def Update_wlsPatch(wls_home,bsu_home,New_wlsPatch,patch_file,version,isremove):
 try:
    ...
    if current_wlsVersion:
        if current_wlsVersion == New_wlsPatch:
           ...
           Install_wlsPatch(wls_home,bsu_home,New_wlsPatch,patch_file,version,isremove)
...
def main():
    ...
    if mode==modes[7]:
       ...
       if New_wlsPatch=='' or patch_file=='':
          ...
          version='1'
          Update_wlsPatch(wls_home,bsu_home,New_wlsPatch,patch_file,version,isremove)
    elif mode==modes[4]:
